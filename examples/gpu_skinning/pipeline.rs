...
const VERTEX: &str = r#"
        ...
        void main() {
            ...
            int slot_index = attachment_slots[uint(attachment_index)];
            int bone_index = slot_bones[uint(slot_index)];
            int deform_offset = deform_offsets[uint(slot_index)];
...
pub fn create_pipeline(ctx: &mut Context) -> Pipeline {
    ...
    let shader = Shader::new(ctx, VERTEX, FRAGMENT, shader_meta).expect("failed to build shader");
